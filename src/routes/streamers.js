...
router.post("/", requireAuth, async (req, res) => {
  ...
  writeFileSync(
    "./data/streamers.json",
    JSON.stringify(streamersData, null, 2)
  );

  try {
    const user = await twitchClient.getUserByName(streamer_name);
    await twitchClient.createOnlineWebhookSubscription(user.id);
    console.log(`---\nAdded subscription for ${streamer_name}\n---`);
  } catch (error) {
    console.error("Error creating subscription:", error);
    streamersData = streamersData.filter(
      (s) => s.streamer_name !== streamer_name
    );
    writeFileSync(
      "./data/streamers.json",
      JSON.stringify(streamersData, null, 2)
...
router.put("/", requireAuth, async (req, res) => {
  ...
  const streamerIndex = streamersData.findIndex(
    (s) => s.streamer_name === oldName
  );
  if (streamerIndex === -1) {
  ...
  if (oldName !== newName) {
    try {
      ...
      const oldSub = subs.data.find(
        (sub) => sub.condition.broadcaster_user_id === oldUser.id
      );
      ...
      console.log(
        `---\nUpdated subscription for ${oldName} -> ${newName}\n---`
      );
    } catch (error) {
      console.error("Error updating subscription:", error);
      streamersData[streamerIndex] = {
        streamer_name: oldName,
        going_live_message: newMessage,
      };
      writeFileSync(
        "./data/streamers.json",
...
router.delete("/", requireAuth, async (req, res) => {
  ...
  const streamerIndex = streamersData.findIndex(
    (s) => s.streamer_name === streamerName
  );
  if (streamerIndex === -1) {
    return res.status(404).json({ error: "Streamer not found" });
  }
  ...
  try {
    ...
    console.log(`---\nDeleted subscription for ${streamerName}\n---`);
  } catch (error) {
